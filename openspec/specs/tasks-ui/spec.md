# Spec de capacidad: `tasks-ui` (Vista de tareas)
...
La aplicación MUST ofrecer un mecanismo de navegación explícito para acceder a la vista de tareas. Dado que los endpoints de tareas son sin autenticación, la navegación MUST estar disponible tanto desde la pantalla de login como desde el área autenticada. La vista de tareas MUST ser una vista diferenciada (no mezclada con el perfil de usuario).

#### Scenario: Navegar a la vista de tareas desde la pantalla de login

- GIVEN que el usuario está en la pantalla de login (sin sesión activa)
...
#### Scenario: Navegar a la vista de tareas desde el área autenticada

- GIVEN que el usuario ha iniciado sesión y está en el área autenticada (perfil)
- WHEN hace clic en el elemento de navegación hacia tareas (identificable por `data-testid="nav-tasks"`)
- THEN la vista de tareas se muestra
- AND el formulario de creación de tareas y la lista de tareas son visibles
